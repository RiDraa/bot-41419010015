...
const token = '5019360992:AAHp3nLbFCO5hle6XcqgMsjfFL0C5UnnWL4'
    ...
    if(state == 1){
        ...
        model.predict(
            [
                ...
                bot.sendMessage(
                    msg.chat.id,
                    `Berikut prediksi y1 adalah ${jres[0]}`
                );
                bot.sendMessage(
                    msg.chat.id,
                    `Berikut prediksi y2 adalah ${jres[1]}`
                ); 
                bot.sendMessage(
                    msg.chat.id,
                    `Berikut prediksi y3 adalah ${jres[2]}`
                );
        })
    }else{
        state = 0;
